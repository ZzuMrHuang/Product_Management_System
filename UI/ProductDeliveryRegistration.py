...
class ProductDeliveryRegistration(MySearchWidgetPIM):
    def __init__(self):
        ...
        self.sqlt = "ProductName,ProductCode,Counts,(select sum(Counts) FROM T_ProductTask_DeliveryAcceptance)sum"
    ...
    def searchButtonEventD(self):
        ...
        if content == "":
            ...
            sqlt = "ProductName,ProductCode,Counts,(select sum(Counts) FROM T_ProductTask_DeliveryAcceptance %s)sum" % (sql)
            self.queryModel.searchTableD(sqlt,sql)
            self.updateUI()
