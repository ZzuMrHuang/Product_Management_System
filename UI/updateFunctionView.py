...
class updateFunctionWidget(object):
    ...
    def conserveButtonEvent(self, Dialog):
        ...
        #                  % (userID,psw,rightID,changeDate,changerId)
    ...
    def updateButtonEvent(self, queryModel):
        ...
        if functionId== "" or chineseName== "":
            ...
            self.query.exec(insert_sql)
            self.db.commit()
            confirm = QMessageBox.information(QDialog(), "提示", "业务信息更新成功！", QMessageBox.Yes, QMessageBox.Yes)
            if confirm == QMessageBox.Yes:
                self.dialog.close()
    #检验输入，合理性
    def checkOn(self,functionId,chineseName,isValid):
        """
        检查用户ID和权限ID是否合理
        :return:
        """
        ...
        self.query.exec(sql)
        if self.query.next():
            print(QMessageBox.warning(QDialog(), "警告", "此业务名称已存在，请更换后重新添加！", QMessageBox.Yes, QMessageBox.Yes))
            return 0
