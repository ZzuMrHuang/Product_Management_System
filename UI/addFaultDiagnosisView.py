...
class AddFaultDiagnosisWidget(object):
    def setupUi(self, Dialog):
        ...
        self.FaultDiagnosticProgram = QtWidgets.QLineEdit(Dialog)
        self.FaultDiagnosticProgram.setObjectName("FaultDiagnosticProgram")
        self.formLayout.setWidget(5, QtWidgets.QFormLayout.FieldRole, self.FaultDiagnosticProgram)
    ...
    def conserveButtonEvent(self, Dialog):
        ...
        FaultDiagnosticProgram = self.FaultDiagnosticProgram.text()
        remark = self.remark.toPlainText()

        # 如果必要的信息都不为空
        if batchNO == "" or FaultType == "" or FaultDiagnosticProgram == "" :
            ...
            insert_sql = "INSERT INTO T_Fault_Diagnosis VALUES ('%s', '%s','%s','%s','%s')" % \
                         ( batchNO,  FaultType,FaultName, FaultDiagnosticProgram, remark)
    ...
    def updateData(self, list, queryModel):
        ...
        self.FaultDiagnosticProgram.setText(list[3])
